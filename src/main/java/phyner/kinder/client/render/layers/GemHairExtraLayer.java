...
@Environment(value = EnvType.CLIENT) public class GemHairExtraLayer<T extends PearlEntity> extends GeoRenderLayer<T> {
    ...
    public void render (MatrixStack poseStack, T gem, BakedGeoModel bakedModel, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay){
        ...
        if (gem.getHairExtraVariant () != 0) {
            texture = new Identifier (KinderMod.MOD_ID, "textures/entity/gems/" + gem.getType ().getUntranslatedName () + "/hair_extra/extra_" + gem.getHairExtraVariant () + ".png");
        } else {
